...
function fullscreen(x) {
    var chart_id = $(x).data('widget-id'),
        content = $('#' + chart_id).html();

    $('#lightbox').fadeIn(300);

    get_chart[chart_id].options.chart.renderTo = 'lightbox_data';

    new Highcharts.Chart(get_chart[chart_id].options)
}

function chart_widget(x) {


$('#'+x.id).html('<center><div class="CW_chart_loading" ></div></center>')
	var html = '',
		x_axis = [],
		series_data = [],
		id, chart, zoom_button, table_button, title_url;
	...
	if(x.build_id){
		...
		if(x.tmpl && x.tmpl_render) {
			html = x.tmpl_render(x.tmpl, x);
		}else{
        var id= x.id 
     
        var chart_info = x;
        var html = '<li class="ui-state-default" id="widget-' + x.id + '">';
        html += '<div id="' + x.id + '" class="widget" style="display:none1" >';

        var zoom_button = '<button class="fullscreenbtn btn btn-primary " data-widget-id="' + x.id + '"onclick="fullscreen(this)"style="float:right;margin-top:5px;margin-right:5px;"><i class="icon-zoom-in icon-white"></i></button>' 
         var table_button = '<button class="fullscreenbtn btn btn-primary " data-widget-id="' + x.id + '"onclick="fullscreen(this) "style="float:right;margin-top:5px;margin-right:5px;"><i class="icon-th-list"></i></button>'
         if(x.title_url){  var title_url = '<a href="'+x.title_url+'">' + x.title +' </a>'}else{ var title_url=x.title }
        html += '<h2 id="' + x.id + '-title">  ' + title_url + zoom_button+ ' </h2>';
        html += '<div id="total_' +id + '"></div> ';
            html += '<div id="' + x.id + '-chart" class="chart1" style="min-width: 200px; height: 300px;max-height: 800px;min-width:100%;display:inline-block;max-width:100%"></div>';
    ...
    if(x.target_id) {
        if(x.replace_target) {
            $('#'+x.target_id).html(html);
        } else {
            $('#'+x.target_id).append(html);
        }
    } else {
        $('#main').append(html);
    }
	...
	$.getJSON(x.url, function (jsondata) {
		...
		var series_data, b, minTickInterval,
			data, series_data, key_id,
			chart_series_data, chart_more_data,
			abbrNum, chart_config, total;
		...
		series_data = [];
	       ...
	       if(jsondata.length  <= 15){
                ...
                /*
            if(jsondata.length  >= 10   && jsondata.length  <= 31)
                 { var minTickInterval =10 }

               if(jsondata.length  >= 32  && jsondata.length  <= 55 )
               {  var minTickInterval =25 }               
           if(jsondata.length  >= 55 )
               {  var minTickInterval =35 }
        */
        if (x.reverse ==='true' && jsondata.length  >1 ) {
            jsondata.reverse();
        }
		...
		//minTickInterval =0;
		for (x in jsondata) {
           //x_axis.push(new Date(jsondata[x][b.x_axis]).getTime() / 1000 );
			x_axis.push(jsondata[x][b.x_axis] );
		}
        ...
        if (b.series[0].id) {
        	...
        	for (c in b.series) {
        		...
        		for (var key in jsondata) {
        			if (jsondata[key][b.series[c].id]) {
        				key_id[[jsondata[key][b.series[c].id]]] = jsondata[key][b.series[c].id];
                        //console.log(key_id)
                    }
                }
                ...
                for (x in key_id) {
                	...
                	for (y in jsondata) {
                		if (jsondata[y][b.series[c].id] === key_id[x]) {

                			chart_series_data.push(parseFloat(jsondata[y][b.series[c].data]))
                            // console.log(json[y])
                        }
                    }
                    series_data.push({
                    	name: "" + key_id[x] + " " + b.series[c].name,
        	...
        	for (d in b.series) {
        		chart_series_data = [];
        		chart_more_data = b.series[d].data;

        	for (var t=0;t<jsondata.length;t++){
              if(b.chart_type === 'pie'){
                        series_data.push( ["" + jsondata[t][b.x_axis] + "",parseFloat(jsondata[t][b.series[d].data])
                ]);
                                }else{
                            chart_series_data.push(parseFloat(jsondata[t][b.series[d].data]));
        ...
        console.log('chart_series_data',series_data); 
        console.log('chart_series_data_id',id); 
        chartConfig = {
            ...
            plotOptions: {
               ...
               tooltip: {
                formatter: function () {
                    ...
                    return '<b>' + this.key + '</b> <br/> Value: ' + addCommas(this.y);
                }
            },
            series: [{
                type: 'pie',
...
/*
        if(!b.total){
        	...
        	for (x in series_data){
        		total = series_data[x].data.reduce(function(a, b) {
        			return  + a + b;
        		});

        		console.log(b.id+' : '+total)
        		overview += '<b> '+series_data[x].name +'</b> : '+ abbrNum(Math.round(total),2)+' | '
        	}
        	$('#total_' +id ).append(overview);
        }
*/
